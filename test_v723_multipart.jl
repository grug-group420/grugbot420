#!/usr/bin/env julia
# v7.23 Multipart Decoherence Test — loads specimen, asks multipart questions, logs to md
using GrugBot420
const GB = GrugBot420
...
println(md, "**Engine version:** v7.23 (confidence-only gating, no stochastic coinflips)  ")
println(md, "\n---\n")
println(md, "## Purpose\n")
println(md, "Verify response-level decoherence fixes in GrugBot420 engine v7.23.  ")
println(md, "Math questions should produce math answers; compound questions should produce coherent per-clause responses.  ")
println(md, "The v7.23 fix removes ALL stochastic strength-biased coinflips from the orchestration pipeline.  ")
println(md, "Confidence is now the ONLY gate: `strength_biased_scan_coinflip` always returns true, ")
println(md, "`strength_biased_vote_coinflip` always returns true, and `SPARSE_ACTIVE_FIRE_FLOOR` is set to 0.0.  ")
println(md, "The real gate is `AIML_TOP_LOCKIN_FLOOR` (0.50) in the orchestration phase.\n")
println(md, "---\n")
...
println(md, "### v7.23 Fixes Verified\n")
println(md, "1. **Stochastic coinflips removed** — `strength_biased_scan_coinflip` and `strength_biased_vote_coinflip` now always return `true`. No node or vote is randomly excluded based on strength.")
println(md, "2. **SPARSE_ACTIVE_FIRE_FLOOR set to 0.0** — Pre-scan confidence floor no longer culls weak matches. `should_fire_sparse_active()` always returns true for finite confidence.")
println(md, "3. **Confidence is the ONLY gate** — The authoritative threshold is `AIML_TOP_LOCKIN_FLOOR` (0.50). Only votes with confidence+linkage >= 0.50 are \"locked in\" and proceed to orchestration.\n")
...
write("multipart_test_log_v723.md", content)
println("\n✅ Log written to multipart_test_log_v723.md")
