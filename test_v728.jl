#!/usr/bin/env julia
# v7.28 Multipart Test — loads specimen, runs multipart questions, logs HTML to md
...
log_line(h1("v7.28 Mutual Incompleteness Test Log"))
log_line(p("$(strong("Generated:")) $(ts)$(br())$(strong("Specimen:")) $(SPECIMEN)"))
log_line(hr())
log_line(h2("Purpose"))
log_line(p("Verify v7.28 mutual incompleteness orchestration rule. When 2+ lobes each have at least one lock-in vote, they get co-equal standing regardless of overall averages. No secondary demotion. curved_avg still decides speaking order. Math questions should produce math answers. Compound questions should produce coherent per-clause responses."))
log_line(hr())
...
# v7.28 verified features
log_line(h3("v7.28 Features Verified"))
log_line("<ol>")
log_line(" <li><strong>Mutual incompleteness detection</strong> — 2+ lobes with ≥1 lock-in each get co-equal standing</li>")
log_line(" <li><strong>Coequal vote bucketing</strong> — all coequal lobe votes go into winner bucket, no secondary demotion</li>")
log_line(" <li><strong>curved_avg speaking order</strong> — floor winner still speaks first among coequals</li>")
log_line(" <li><strong>Multipart decomposition</strong> — compound inputs split into clauses with per-clause objective IDs</li>")
log_line(" <li><strong>Sigil routing</strong> — math sigils inject nodes with guaranteed confidence floor</li>")
log_line("</ol>")
log_line(p("$(strong("✅ ALL TESTS PASSED — Mutual incompleteness verified."))"))

# === WRITE LOG ===
log_path = "v728_test_log.md"
