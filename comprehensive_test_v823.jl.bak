#!/usr/bin/env julia
# ==============================================================================
# comprehensive_test_v823.jl — Full GrugBot420 v8.23 comprehensive test
# ==============================================================================
...
#         /answer teach-and-reask (multiple lobes), relational sigils,
#         thesaurus variation checks, node type coverage
# Logs results to MD file using in-program capture (not stdout)
# Saves specimen after /answer teaching
# ==============================================================================
...
const LOG_PATH   = "/workspace/comprehensive_test_v823_log.md"
const SAVE_PATH  = "/workspace/test_v823_saved.specimen"
...
function run_test(category::String, query::String; expect_ask::Bool=false)
    ...
    elseif is_failure_response(answer) && !expect_ask
        ...
        verdict = "CAVE-EMPTY"
    elseif is_failure_response(answer) && expect_ask
        # ASK responses contain "Nothing in the cave" — that's expected
        status = "✅ ASK"
        verdict = "ASK generated"
...
function _teach_answer(lobe::String, mode::String, content::String)
    """
    Teach an /answer node using internal API calls. Returns (primary_id, shadow_ids).
    Replicates the CLI /answer mode dispatch logic from Main.jl lines 12874-13350.
    """
    # Validate lobe
    ...
    try
        ...
        @warn "dampen_strain! failed (non-fatal)" exception=e
    end
    ...
    if mode_str == "relate"
        ...
        relate_parts = [strip(String(p)) for p in split(content, "|")]
        ...
        obj  = relate_parts[3]
        rel_for_triple = rel_raw  # keep &sigil form if present

        relate_data = _base_answer_data("reason"; pending_ask_text=pending_ask_text,
                                         answer_content="$(lowercase(subj)) $(rel_for_triple) $(lowercase(obj))")
        ...
        primary_id, shadow_ids, lobe_tag = _plant_answer_cluster(
            subj, "reason^1", relate_data, target_lobe, "relate")
        return (primary_id, shadow_ids)

    elseif mode_str == "time"
        # Format: "subject | object [| orientation]"
        time_parts = [strip(String(s)) for s in split(content, "|")]
        ...
        subj = time_parts[1]
        obj  = time_parts[2]
        time_orient = length(time_parts) >= 3 ? strip(time_parts[3]) : ""
        ...
        primary_id, shadow_ids, lobe_tag = _plant_answer_cluster(
            subj, "reason^1", time_data, target_lobe, "time")
        return (primary_id, shadow_ids)

    elseif mode_str == "proc"
        # Format: "step1; step2; step3"
        proc_steps = [strip(String(s)) for s in split(content, ";")]
        ...
        for (i, step_text) in enumerate(proc_steps)
            step_text_str = String(step_text)  # ensure String, not SubString
            step_data = _base_answer_data("reason"; pending_ask_text=pending_ask_text,
                                           answer_content=step_text_str)
            ...
            nid, lobe_tag = _create_answer_node(step_text_str, "reason^1", step_data, target_lobe; skip_auto_latch=true)
            push!(proc_ids, nid)
        end
        # Link steps into drop_table chain
        for i in 1:(length(proc_ids)-1)
            if haskey(NODE_MAP, proc_ids[i])
                push!(NODE_MAP[proc_ids[i]].drop_table, proc_ids[i+1])
            end
        end
        # Auto-group
        if length(proc_ids) > 1
            try
                ...
                @warn "/answer :proc auto-grouping failed (non-fatal)" exception=e
            end
        end
        return (proc_ids[1], proc_ids[2:end])

    elseif mode_str == "multi"
        # GRUG v8.25: :multi mode now creates FULL CLUSTERS (primary + fan-out shadows)
        # per part, just like how multipart QUESTIONS split into sub-missions.
        # Each part gets _plant_answer_cluster which generates query-matching
        # shadow patterns ("what is X", "tell me about X", etc.) so the scanner
        # can find the answer when the user re-asks the original question.
        # Before: _create_answer_node(part_text, ...) — pattern was the content
        #   text ("mitosis is cell division..."), which never matched queries.
        # After:  _plant_answer_cluster(part_text, ...) — creates shadows like
        #   "what is mitosis", "mitosis", etc. that DO match queries.
        raw_parts = split(content, "|")
        parts = Tuple{String, String, String}[]  # (action_pkt, part_text, part_mode)
        for part in raw_parts
            trimmed = strip(String(part))
            isempty(trimmed) && continue
            m = match(r"^:(\w+)\s+(.+)$", trimmed)
            if !isnothing(m)
                part_mode = lowercase(String(m.captures[1]))
                push!(parts, ("$(part_mode)^1", strip(String(m.captures[2])), part_mode))
            else
                push!(parts, ("reason^1", trimmed, "reason"))
            end
        end
        if isempty(parts)
            @warn "/answer :multi needs pipe-delimited parts"
            return ("", String[])
        end
        ...
        for (i, (action_pkt, part_text, part_mode)) in enumerate(parts)
            # Build answer data for this part using its own mode config
            cfg = get(_ANSWER_MODE_CONFIG, part_mode, _ANSWER_MODE_CONFIG["reason"])
            part_data = _base_answer_data(part_mode; pending_ask_text=pending_ask_text,
                                           answer_content=part_text)
            ...
            # Set voice/frame from the part's own mode config
            ...
            primary_id, shadow_ids, lobe_tag = _plant_answer_cluster(
                part_text, action_pkt, part_data, target_lobe, part_mode)

            push!(all_primary_ids, primary_id)
            append!(all_shadow_ids, shadow_ids)
        end

        # Cross-link all primary nodes from different parts via drop_table
        ...
        # Auto-group ALL clusters together (primaries + all shadows)
        ...
        if length(all_cluster_ids) > 1
            try
                ...
                @warn "/answer :multi auto-grouping failed (non-fatal)" exception=e
            end
        end
    ...
    elseif mode_str == "math"
        ...
        primary_id, shadow_ids, lobe_tag = _plant_answer_cluster(
            content, "reason^1", math_data, target_lobe, "math")
        return (primary_id, shadow_ids)

    else
        # Default modes: reason, explain, define, alert, comfort
        ...
        primary_id, shadow_ids, lobe_tag = _plant_answer_cluster(
            content, action_pkt, typed_data, target_lobe, mode_str)
        return (primary_id, shadow_ids)
    end
end
...
function run_answer_scenario(
    ...
    # Step 2: Teach via internal API (bypasses CLI /answer loop)
    ...
    # Check for content anchors (keywords from the taught content)
    # For modes with special delimiters (relate,time,proc,multi), strip | ; & chars first
...
function run_variation_test(query::String, n_trials::Int=3)
    ...
    # Check if at least 2 responses differ
...
# ── Write MD header ───────────────────────────────────────────────────────────
log_md("# GrugBot420 Comprehensive Test Log v8.25")
log_md("")
log_md("**Date:** $(now())")
log_md("**Specimen:** $SPEC_PATH")
log_md("**Chatter:** DISABLED")
log_md("**Capture method:** _LAST_VOICE_OUTPUT (application internals)")
log_md("**WORD COEFFICIENT:** Active (smooth semantic weight spectrum 0.05–1.0)")
log_md("**v8.25 fix:** :multi mode now creates full clusters with fan-out shadows per part (query-matching patterns)")
log_md("")
log_md("---")
log_md("")
...
# ── Load specimen ─────────────────────────────────────────────────────────────
println("=" ^ 70)
println("GRUGBOT420 COMPREHENSIVE TEST v8.23")
println("=" ^ 70)
...
try
    ...
    # The specimen lobes are at/over capacity. /answer teach needs room.
    # Expand each lobe to accommodate new nodes.
    for (lobe_name, rec) in LOBE_REGISTRY
        alive = count_alive_nodes_in_lobe(lobe_name)
        needed = alive + 40  # room for new answer nodes per lobe (including :proc/:multi multi-node)
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 5: MATH — ARITHMETIC
# ═══════════════════════════════════════════════════════════════════════════════
log_md("# Section 5: Math — Arithmetic")
log_md("")
println("─" ^ 70)
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 13: RELATIONAL SIGILS
# ═══════════════════════════════════════════════════════════════════════════════
log_md("# Section 13: Relational Sigils (Dynamic Happenings)")
log_md("")
println("─" ^ 70)
...
# ═══════════════════════════════════════════════════════════════════════════════
log_md("# /answer Mechanic Test (Teach-and-Reask with Lobes)")

# DEBUG: Check for auto-grown placeholder nodes that might interfere with /answer recall
let
    autogrown = lock(NODE_LOCK) do
        [(id, n.pattern, n.is_grave, get(n.json_data, "autogrowth_source", ""), get(n.json_data, "growth_source", "")) for (id, n) in NODE_MAP if !n.is_grave && !isempty(get(n.json_data, "autogrowth_source", ""))]
    end
    println("[DEBUG] Auto-grown nodes alive: $(length(autogrown))")
    for (id, pat, grave, auto_src, growth_src) in autogrown[1:min(5, length(autogrown))]
        println("  $id: pattern='$pat' auto_src='$auto_src'")
    end
end
log_md("")
println("─" ^ 70)
...
push!(answer_results, run_answer_scenario(
    ...
    "what is a stromatolite",
    "/answer @science :explain stromatolites are layered rock structures formed by cyanobacteria in shallow water",
    "science", "explain",
...
push!(answer_results, run_answer_scenario(
    "what is a quasar",
    "what is a quasar",
    "/answer @science :reason quasars are extremely luminous active galactic nuclei powered by supermassive black holes",
    "science", "reason",
    "quasars are extremely luminous active galactic nuclei powered by supermassive black holes"
))

# Scenario 3: Science lobe, :explain mode — fermentation
push!(answer_results, run_answer_scenario(
    "what is fermentation",
    "what is fermentation",
    "/answer @science :explain fermentation converts sugar into alcohol and carbon dioxide using yeast or bacteria",
    "science", "explain",
    "fermentation converts sugar into alcohol and carbon dioxide using yeast or bacteria"
))

# Scenario 4: Mathematics lobe, :define mode — golden ratio
push!(answer_results, run_answer_scenario(
    "what is the golden ratio",
    "what is the golden ratio",
    "/answer @mathematics :define the golden ratio is approximately 1.618 and appears in art architecture and nature",
    "mathematics", "define",
    "the golden ratio is approximately 1.618 and appears in art architecture and nature"
))

# Scenario 5: Emotion lobe, :reason mode — empathy
push!(answer_results, run_answer_scenario(
    "what is empathy",
    "what is empathy",
    "/answer @emotion :reason empathy is understanding and sharing the feelings of another person through emotional connection",
    "emotion", "reason",
    "empathy is understanding and sharing the feelings of another person through emotional connection"
))

# Scenario 6: Language lobe, :define mode — sonnet
push!(answer_results, run_answer_scenario(
    "what is a sonnet",
    "what is a sonnet",
    "/answer @language :define a sonnet is a fourteen line poem with a specific rhyme scheme and meter",
    "language", "define",
    "a sonnet is a fourteen line poem with a specific rhyme scheme and meter"
))

# Scenario 7: Philosophy lobe, :reason mode — solipsism
push!(answer_results, run_answer_scenario(
    "what is solipsism",
    "what is solipsism",
    "/answer @philosophy :reason solipsism is the philosophical idea that only ones own mind is certain to exist",
    "philosophy", "reason",
    "solipsism is the philosophical idea that only ones own mind is certain to exist"
))

# Scenario 8: History lobe, :explain mode — renaissance
push!(answer_results, run_answer_scenario(
    "what was the renaissance",
    "what was the renaissance",
    "/answer @history :explain the renaissance was a cultural movement in Europe from the 14th to 17th century that revived classical learning",
    "history", "explain",
    "the renaissance was a cultural movement in Europe from the 14th to 17th century that revived classical learning"
))

# Scenario 9: Technology lobe, :define mode — algorithm
push!(answer_results, run_answer_scenario(
    "what is an algorithm",
    "what is an algorithm",
    "/answer @technology :define an algorithm is a step by step procedure for solving a problem or accomplishing a task",
    "technology", "define",
    "an algorithm is a step by step procedure for solving a problem or accomplishing a task"
))

# Scenario 10: Mathematics lobe, :math mode — prime numbers
push!(answer_results, run_answer_scenario(
    "what are prime numbers",
    "what are prime numbers",
    "/answer @mathematics :math prime numbers are natural numbers greater than 1 that have no positive divisors other than 1 and themselves",
    "mathematics", "math",
    "prime numbers are natural numbers greater than 1 that have no positive divisors other than 1 and themselves"
))

# Scenario 11: Nature lobe, :relate mode — water cycle (triple-seeded)
# NOTE: content for :relate must be pipe-delimited "subject | relation | object"
push!(answer_results, run_answer_scenario(
    "what is the water cycle",
    "what is the water cycle",
    "/answer @nature :relate water cycle | &causal causes | evaporation condensation precipitation",
    "nature", "relate",
    "water cycle | &causal causes | evaporation condensation precipitation"
))

# Scenario 12: Emotion lobe, :comfort mode — grief
push!(answer_results, run_answer_scenario(
    "what is grief",
    "what is grief",
    "/answer @emotion :comfort grief is deep sorrow caused by loss and it takes time and support to heal",
    "emotion", "comfort",
    "grief is deep sorrow caused by loss and it takes time and support to heal"
))

# Scenario 13: History lobe, :time mode — temporal relation (sigil &temporal)
# NOTE: content for :time must be pipe-delimited "subject | object [| orientation]"
push!(answer_results, run_answer_scenario(
    "what came before the renaissance",
    "what came before the renaissance",
    "/answer @history :time the dark ages | the renaissance | past",
    "history", "time",
    "the dark ages | the renaissance | past"
))

# Scenario 14: Science lobe, :proc mode — procedure (semicolon-delimited steps)
# NOTE: content for :proc must be semicolon-delimited "step1; step2; step3"
push!(answer_results, run_answer_scenario(
    "how does photosynthesis work",
    "how does photosynthesis work",
    "/answer @science :proc sunlight is absorbed by chlorophyll; water is split into hydrogen and oxygen; carbon dioxide is converted into glucose; oxygen is released as waste",
    "science", "proc",
...
# Scenario 15: Science lobe, :multi mode — multipart answer (pipe-delimited with mode prefixes)
# NOTE: content for :multi must be pipe-delimited ":mode1 text1 | :mode2 text2"
# GRUG v8.25: :multi now creates FULL CLUSTERS per part (_plant_answer_cluster)
# with fan-out shadow nodes that have query-matching patterns ("what is mitosis", etc.)
# so recall works when the original question is re-asked.
push!(answer_results, run_answer_scenario(
    "what is mitosis and why does it matter",
    "what is mitosis and why does it matter",
    "/answer @science :multi :define mitosis is cell division producing two identical daughter cells | :reason mitosis enables growth tissue repair and asexual reproduction",
    "science", "multi",
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 15: THESAURUS VARIATION CHECK (SAME QUERY × 3)
# ═══════════════════════════════════════════════════════════════════════════════
log_md("# Thesaurus Variation Check (Same Query × 3)")
log_md("")
println("─" ^ 70)
...
variation_results = []
push!(variation_results, run_variation_test("what is fire"))
push!(variation_results, run_variation_test("what is water"))
push!(variation_results, run_variation_test("what is gravity"))

# ═══════════════════════════════════════════════════════════════════════════════
# SAVE SPECIMEN AFTER /answer TEACHING
# ═══════════════════════════════════════════════════════════════════════════════
println()
println("Saving specimen (includes /answer-taught nodes)...")
log_md("# Specimen Save")
log_md("")
try
...
answer_pass = count(r -> r[4], answer_results)  # recall_ok (index 4 in 6-tuple)
answer_teach_pass = count(r -> r[3], answer_results)  # teach_ok
...
| /answer recall pass | $answer_pass / $answer_total |")
...
if !isempty(failures)
    log_md("## Items Needing Attention")
    log_md("")
...
math_checks = [
    ...
    ("double 7", "14", "dbl"),
    ("half of 12", "6", "half"),
    ("fibonacci of 10", "55", "fib"),
    ("absolute value of -15", "15", "abs"),
