#!/usr/bin/env julia
# ==============================================================================
# comprehensive_test_v828e.jl — GrugBot420 FULL-FEATURE Specimen Test
# ==============================================================================
# Covers ALL features end-to-end:
#   S1:  Decoherence fixes (v8.28e) — false winners, interrogatives, relational
...
import .GrugBot420.RoutingJudge:
    collect_intents, resolve, compute_graph_backing,
    shannon_entropy, CONSERVATIVE_BIAS, LAZY_ENTROPY_THRESHOLD

# ── Configuration ──────────────────────────────────────────────────────────────
const SPEC_PATH  = get(ARGS, 1, "/workspace/grugbot420_repo/grug_v828_post_test.specimen")
const LOG_PATH   = "/workspace/grugbot420_repo/grug_v828e_comprehensive_test.md"
const SAVE_PATH  = "/workspace/grugbot420_repo/grug_v828e_post_test.specimen"
...
# ── Load specimen ──────────────────────────────────────────────────────────────
println("=" ^ 70)
println("GRUGBOT420 COMPREHENSIVE TEST v8.28e")
println("=" ^ 70)
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 1: DECOHERENCE FIXES (v8.28e) — False Winner Detection
# ══════════════════════════════════════════════════════════════════════════════
...
# The classic false-winner cases that v8.28e fixed
...
# ═════════════════════════════════════════════════════════════════════════════
# SECTION 15: ROUTINGJUDGE — Entropy Jitter, Graph Backing, Feedback Loop (NEW v8.28e)
# ═════════════════════════════════════════════════════════════════════════════
log_md("# Section 15: RoutingJudge — Entropy Jitter, Graph Backing, Feedback Loop")
log_md("")
println("═" ^ 70)
println("SECTION 15: ROUTINGJUDGE")
println("═" ^ 70)
println()

routingjudge_results = []

# 15a: Entropy jitter snap-back
log_md("## RoutingJudge: entropy-jitter-what-time")
log_md("")
_cands_time = collect_intents("what time is it")
log_md("**Input:** \"what time is it\"")
log_md("**Candidates:** $(length(_cands_time))")
for _c in _cands_time
    log_md("- :$(_c.kind) topic='$(_c.topic)' mq=$(_c.match_quality) src=$(_c.source)")
end
_resolved_time = resolve(_cands_time)
_time_ok = _resolved_time !== nothing && first(_resolved_time) == :question
log_md("**Resolved:** $(_resolved_time)")
log_md("**Verdict:** $(_time_ok ? "✅ :question wins (entropy jitter snap-back)" : "❌ Got :$(first(_resolved_time)) instead of :question")")
log_md("---")
log_md("")
println("[routing-judge] entropy-jitter-what-time: $(_time_ok ? "✅" : "❌")")
push!(routingjudge_results, ("entropy-jitter-what-time", _time_ok))

# 15b: Graph backing — known vs unknown
log_md("## RoutingJudge: graph-backing-known-vs-unknown")
log_md("")
_gb_known = compute_graph_backing("fire", :question)
_gb_unknown = compute_graph_backing("xyzzyplugh_nonexistent", :question)
_gb_ok = _gb_known > _gb_unknown
log_md("**Graph backing for 'fire' (known):** $_gb_known")
log_md("**Graph backing for 'xyzzyplugh_nonexistent' (unknown):** $_gb_unknown")
log_md("**Verdict:** $(_gb_ok ? "✅ Known topic has higher graph backing" : "❌ Graph backing not differentiating")")
log_md("---")
log_md("")
println("[routing-judge] graph-backing: $(_gb_ok ? "✅" : "❌")")
push!(routingjudge_results, ("graph-backing-known-vs-unknown", _gb_ok))

# 15c: Pending teach crosstalk — topic mismatch
log_md("## RoutingJudge: pending-teach-topic-mismatch")
log_md("")
_conv_set_pending_teach!("penicillin", "What is penicillin?")
_cands_crosstalk = collect_intents("tides are caused by gravity")
_crosstalk_ok = !any(c -> c.kind == :teach && occursin("penicillin", c.topic), _cands_crosstalk)
log_md("**Pending:** topic='penicillin'")
log_md("**Input:** \"tides are caused by gravity\"")
log_md("**Candidates:** $(length(_cands_crosstalk))")
for _c in _cands_crosstalk
    log_md("- :$(_c.kind) topic='$(_c.topic)' src=$(_c.source)")
end
log_md("**Verdict:** $(_crosstalk_ok ? "✅ No false :teach for penicillin" : "❌ False :teach for penicillin detected")")
log_md("---")
log_md("")
_conv_clear_pending_teach!()
println("[routing-judge] pending-teach-topic-mismatch: $(_crosstalk_ok ? "✅" : "❌")")
push!(routingjudge_results, ("pending-teach-topic-mismatch", _crosstalk_ok))

# 15d: Pending teach with matching topic
log_md("## RoutingJudge: pending-teach-matching-topic")
log_md("")
_conv_set_pending_teach!("factorial", "What does factorial mean?")
_cands_match = collect_intents("math, factorial is multiply all numbers from 1 to n")
_resolved_match = resolve(_cands_match)
_match_ok = _resolved_match !== nothing && first(_resolved_match) == :teach
log_md("**Pending:** topic='factorial'")
log_md("**Input:** \"math, factorial is multiply all numbers from 1 to n\"")
log_md("**Candidates:** $(length(_cands_match))")
for _c in _cands_match
    log_md("- :$(_c.kind) topic='$(_c.topic)' src=$(_c.source)")
end
log_md("**Resolved:** $(_resolved_match)")
log_md("**Verdict:** $(_match_ok ? "✅ :teach wins for matching topic (with lobe hint)" : "❌ Got :$(first(_resolved_match)) instead of :teach")")
log_md("---")
log_md("")
_conv_clear_pending_teach!()
println("[routing-judge] pending-teach-matching-topic: $(_match_ok ? "✅" : "❌")")
push!(routingjudge_results, ("pending-teach-matching-topic", _match_ok))

# 15e: Ack detection
log_md("## RoutingJudge: ack-detection-yes")
log_md("")
_conv_set_pending_teach!("orogeny", "What is orogeny?")
_cands_ack = collect_intents("yes")
_ack_ok = !any(c -> c.kind == :teach, _cands_ack)
log_md("**Pending:** topic='orogeny'")
log_md("**Input:** \"yes\"")
log_md("**Candidates:** $(length(_cands_ack))")
for _c in _cands_ack
    log_md("- :$(_c.kind) topic='$(_c.topic)' src=$(_c.source)")
end
log_md("**Verdict:** $(_ack_ok ? "✅ 'yes' not treated as :teach" : "❌ 'yes' incorrectly treated as :teach")")
log_md("---")
log_md("")
_conv_clear_pending_teach!()
println("[routing-judge] ack-detection-yes: $(_ack_ok ? "✅" : "❌")")
push!(routingjudge_results, ("ack-detection-yes", _ack_ok))

# 15f: Feedback loop
log_md("## RoutingJudge: feedback-loop-graph-backing-increase")
log_md("")
_feedback_topic = "grugtest_$(round(Int, time()))"
_gb_before = compute_graph_backing(_feedback_topic, :question)
log_md("**Topic:** $_feedback_topic")
log_md("**Graph backing BEFORE:** $_gb_before")
_conv_clear_pending_teach!()
_fb_raw1 = run_mission("what is $(_feedback_topic)")
_fb_ans1 = clean_output(_fb_raw1)
log_md("**Step 1 — Ask:** $(first(_fb_ans1, 150))")
_fb_raw2 = run_mission("science, $(_feedback_topic) is a rare mineral found in volcanic deposits")
_fb_ans2 = clean_output(_fb_raw2)
log_md("**Step 2 — Teach:** $(first(_fb_ans2, 150))")
_gb_after = compute_graph_backing(_feedback_topic, :question)
log_md("**Graph backing AFTER:** $_gb_after")
_fb_loop_ok = _gb_after > _gb_before
log_md("**Verdict:** $(_fb_loop_ok ? "✅ Feedback loop verified" : "❌ Graph backing did not increase")")
log_md("---")
log_md("")
println("[routing-judge] feedback-loop: $(_fb_loop_ok ? "✅" : "❌")")
push!(routingjudge_results, ("feedback-loop-graph-backing-increase", _fb_loop_ok))

# 15g: "what causes X" pattern
log_md("## RoutingJudge: what-causes-X-pattern")
log_md("")
_cands_causes = collect_intents("what causes tides")
_resolved_causes = resolve(_cands_causes)
_causes_ok = _resolved_causes !== nothing && first(_resolved_causes) == :question
log_md("**Input:** \"what causes tides\"")
log_md("**Candidates:** $(length(_cands_causes))")
for _c in _cands_causes
    log_md("- :$(_c.kind) topic='$(_c.topic)' src=$(_c.source)")
end
log_md("**Resolved:** $(_resolved_causes)")
log_md("**Verdict:** $(_causes_ok ? "✅ 'what causes X' detected as :question" : "❌ Got :$(first(_resolved_causes))")")
log_md("---")
log_md("")
println("[routing-judge] what-causes-X: $(_causes_ok ? "✅" : "❌")")
push!(routingjudge_results, ("what-causes-X-pattern", _causes_ok))

# 15h: :correct beats :define
log_md("## RoutingJudge: correct-over-define")
log_md("")
_cands_correct = collect_intents("no fire is plasma not oxidation")
_resolved_correct = resolve(_cands_correct)
_correct_ok = _resolved_correct !== nothing && first(_resolved_correct) == :correct
log_md("**Input:** \"no fire is plasma not oxidation\"")
log_md("**Resolved:** $(_resolved_correct)")
log_md("**Verdict:** $(_correct_ok ? "✅ :correct wins over :define" : "❌ Got :$(first(_resolved_correct))")")
log_md("---")
log_md("")
println("[routing-judge] correct-over-define: $(_correct_ok ? "✅" : "❌")")
push!(routingjudge_results, ("correct-over-define", _correct_ok))

# 15i: Lazy mode greeting
log_md("## RoutingJudge: lazy-mode-greeting")
log_md("")
_cands_greet = collect_intents("hello there")
_greet_ok = isempty(_cands_greet) || (length(_cands_greet) == 1 && first(_cands_greet).kind == :nothing)
log_md("**Input:** \"hello there\"")
log_md("**Candidates:** $(length(_cands_greet))")
for _c in _cands_greet
    log_md("- :$(_c.kind) topic='$(_c.topic)'")
end
log_md("**Verdict:** $(_greet_ok ? "✅ Greeting produces nothing (lazy)" : "⚠️ Unexpected candidates")")
log_md("---")
log_md("")
println("[routing-judge] lazy-mode-greeting: $(_greet_ok ? "✅" : "⚠️")")
push!(routingjudge_results, ("lazy-mode-greeting", _greet_ok))


# ═════════════════════════════════════════════════════════════════════════════
# SECTION 16: RESPONSE QUALITY — No-Dumb-Response Audit (NEW v8.28e)
# ═════════════════════════════════════════════════════════════════════════════
log_md("# Section 16: Response Quality — No-Dumb-Response Audit")
log_md("")
println("═" ^ 70)
println("SECTION 16: RESPONSE QUALITY AUDIT")
println("═" ^ 70)
println()

quality_results = []

function audit_response(label::String, query::String; 
                         must_contain::String="",
                         must_not_contain::Vector{String}=String[],
                         must_not_match::Regex=r"NEVER_MATCH",
                         min_length::Int=5)
    ...
    _issues = String[]
    if isempty(_ans)
        push!(_issues, "EMPTY response")
        _ok = false
    elseif length(_ans) < min_length
        push!(_issues, "Response too short ($(length(_ans)) chars, min=$min_length)")
        _ok = false
    end
    if !isempty(must_contain) && !occursin(lowercase(must_contain), lowercase(_ans))
        push!(_issues, "Missing expected content: '$must_contain'")
        _ok = false
    end
    for bad in must_not_contain
        if occursin(lowercase(bad), lowercase(_ans))
            push!(_issues, "Contains forbidden: '$bad'")
            _ok = false
        end
    end
    if occursin(must_not_match, _ans)
        push!(_issues, "Matches forbidden pattern: $must_not_match")
        _ok = false
    end
    if occursin("Grug shrugs", _ans) || occursin("cave is empty", _ans)
        push!(_issues, "Generic failure response")
        _ok = false
    end
    log_md("**Output:** $(first(_ans, 300))")
    if isempty(_issues)
        log_md("**Verdict:** ✅ Response quality OK")
    else
    ...
    log_md("---")
    log_md("")
    println("[quality] $label: $(_ok ? "✅" : "❌") $(_ok ? "" : join(_issues, ", "))")
    return (label, _ok, _ans, _issues)
end

# 16a: Known questions — substantive answers
push!(quality_results, audit_response("known-question-fire",
    "what is fire";
    must_not_contain=["Grug not know fire"],
    must_not_match=r"\bwhat does fire mean\b"i))

push!(quality_results, audit_response("known-question-water",
    "what is water";
    must_not_contain=["Grug not know water"]))

# 16b: Emotion — no definition-style answers
push!(quality_results, audit_response("emotion-no-def-style",
    "how are you feeling";
    must_not_contain=["📖 how means", "📖 feeling means"],
    must_not_match=r"\bhow\s+(means|is)\b"i))

# 16c: Corrections acknowledged, not treated as new definitions
push!(quality_results, audit_response("correction-acknowledged",
    "no fire is plasma not oxidation";
    must_not_contain=["📖 Learned: no fire means"]))

# 16d: "what causes tides" — substantive answer
push!(quality_results, audit_response("what-causes-substantive",
    "what causes tides";
    min_length=10))

# 16e: "why is the sky blue" — explains scattering
push!(quality_results, audit_response("why-sky-blue-substantive",
    "why is the sky blue";
    must_contain="scatt",
    min_length=20))

# 16f: Greetings — friendly, not robotic
push!(quality_results, audit_response("greeting-friendly",
    "hello";
    must_contain="Grug",
    must_not_contain=["Nothing in the cave", "Grug shrugs"],
    min_length=10))

# 16g: Math correctness
push!(quality_results, audit_response("math-correctness",
    "3 + 5";
    must_contain="8",
    min_length=1))

# 16h: Philosophy — thoughtful, not empty
# NOTE: "Grug not know" in a philosophical context is epistemic humility, not failure.
# e.g. "Grug not know where it comes from but Grug knows it burns" is GOOD.
push!(quality_results, audit_response("philosophy-thoughtful",
    "what is consciousness";
    min_length=20))

# 16i: Define then ask retrieves definition
_conv_clear_pending_teach!()
_def_audit_raw = run_mission("sintering is a process of heating powder to just below melting point")
_def_audit_ans = clean_output(_def_audit_raw)
push!(quality_results, audit_response("define-then-ask",
    "what is sintering";
    must_contain="sintering",
    must_not_contain=["Grug not know sintering"]))

# 16j: "where" question — no false definition
push!(quality_results, audit_response("where-question-no-def",
    "where do rivers come from";
    must_not_contain=["📖 where means", "📖 where is"],
    must_not_match=r"\bwhere\s+(means|is)\b"i))

# 16k: Self-reference — no Fire swapping
push!(quality_results, audit_response("no-fire-swap",
    "who are you";
    must_not_contain=["Fire is", "Fire am", "Fire sit"],
    must_not_match=r"\bFire\b.*\bam\b"i))
...
global _all_collections = [
    ...
    ("RoutingJudge (S15)", routingjudge_results),
    ("Response Quality (S16)", quality_results),
]
