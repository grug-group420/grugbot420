...
- *current* — sweep-cleaning pass. Removed the sigil-bracketing
  requirement (macros are just regex-extract-and-substitute; the
  `%X%` / `&X&` / `@X@` framing was a leftover from when macros had
  `kind ∈ {token, functor, both}` dispatching on the sigil). Removed
  the Hopfield-familiarity exposition (the cache is commented out;
  one note in §1.1 is enough). Removed vestigial sections that
  described the macro plan in terms of what it *isn't* relative to
  prior revisions nobody will read (relational-delta meta-arrows
  framing, snap-rounding hygiene that admitted it had no macro
  sites, comparison tables and forensic "removed" footers).
- *earlier revisions* — explored functor-based macros, coinflip-
  candidate macros, sigil-tagged sum-type variants, sandboxed
  Julia REPL eval with lambda meta-programming, two-channel render
  with `MacroFact` envelope, macros-as-`COMMANDS`-entries. All
  superseded; see git history (`326fdb0`, `dd92159`, `0e940aa`)
  for the architectural derivations.
...
The user's exact final framing, in three pieces:
...
1. *"what registered macros should do is regex on the user input to
   ...
   first place. no functor or whackyness needed."*
2. *"there should also be a general %reflect%(pattern to look up in
   memory hash table derived from user input regex). aiml before it
   even gets the macro the macro is resolved to its coherent value.
   aiml doesnt even have to plugin really."*
3. *"because its going by pattern bind identification of structures
   uses normal methods we already have. its just votes can now
   optionally have macro enforcement."*

These three sentences specify the entire feature. The macro system
is **regex-extract-or-memory-lookup, resolve to string, weave like
any other support_piece**. Pattern-bind is reused as-is; nothing
new is invented for activation. AIML is unchanged. Votes gain one
optional field carrying enforcement directives.

---

## 1. Code-level reality check (verified against fresh HEAD `0e940aa`)
...
mutable struct Node
    ...
    hopfield_key::UInt64                 # legacy; cache is disabled (engine.jl:2453 commented out)
...
**Macros are NOT COMMANDS entries.** `COMMANDS` continues to hold
only grug-defined action families (`reason`, `greet`, `inform`,
etc.). Macro resolution is a separate dispatch step that runs
*between* the primary handler and AIML synthesis — see §4.
...
### 1.9 Complexity-driven scan mode and dynamic relational triples (`engine.jl:181`, `engine.jl:2371`)
...
reference compound, nested, or causal structure.
...
3. Mode-3 failure is **fatal** (`@error` + rethrow). The engine
   ...
   Mode-1/2 failure is non-fatal (warn + empty triples).

**Why this matters for macros:** the voter's `relational_patterns`
match against the extracted triples. Triples extracted by
`extract_dynamic_relational_triples` include shapes that
`extract_relational_triples` cannot produce (causal chains,
multi-clause structures). So a voter whose relational pattern is
`(X, causes, (Y, causes, Z))` **only activates on inputs complex
enough to trigger mode 3**. That activation pathway is implicit,
deterministic, and zero-cost from the macro side — the voter's
relational pattern acts as a complexity gate, and any macro tail it
declares fires only when the input is genuinely complex.

This means the §5 activation-pathway list distinguishes basic-triple
matching from dynamic-triple matching. The macro plug-in is
transparent to `screen_input_complexity`, `_effective_scan_mode`,
and `extract_dynamic_relational_triples`; no engine code in this
plan touches them.
...
needs surfacing"). The voter author writes the broad pattern once on
the voter and writes the fine pattern once on each macro. Both are
reusable across many voters once registered.
...
3. Macro: `reflect`'s regex captures `"turtles"` from the macro
   invocation's own arg-string `"(turtles)"` (the regex can target
   the arg-string via scope `:arg` — see §3.3 — or against the user
   input directly via `:current_input`). The captured key looks up
   `"turtles"` against `MESSAGE_HISTORY` (the Memory Cave) — finds
   prior `ChatMessage`s where the text matches. Resolution returns
   the matching message text. Template substitutes that text in.
...
```julia
struct MacroSpec
    action_name :: String          # e.g. "calculate" — what voters write after >>
...
Five fields plus notes. No closures, no functors, no arg-parsers,
no kinds.
...
- `action_name` non-empty, no whitespace, contains no action-packet
  metasyntax (`[`, `]`, `|`, `^`, `>`, `(`, `)`).
...
`_swap_words_in`. Six lines of glue.
...
AIML synthesis sees only fully resolved strings in its
`support_pieces` vector. The macro's wrapper (e.g. `calculate(...)`)
never reaches AIML; it was already substituted out in beat 2.
There's no rule-board substitution pass, no skeleton-aware clause
dispatcher, no envelope unpacking. AIML weaves resolved strings the
same way it weaves triple-derived strings.
...
- Relational triples — **basic** (subject-verb-object) on simple
  ...
  as a complexity-gated activator. See §1.9.
...
There is no built-in macro that does actual arithmetic. `calculate`
extracts `"2+2"` as a substring; if the user wants the bot to *say*
`4`, they handle that downstream of macros (e.g. an AIML rule that
...
- Optional `:relational_triples` scope (regex against the carrier
  `Vote.user_triples` so macros can surface complex relational
  structure that mode-3 dynamic extraction has already produced).
...
The shape this plan commits to, stated as constraints rather than
forensic rejections:

- **Macros are not `COMMANDS` entries.** `COMMANDS` holds
  grug-defined action families (`reason`, `greet`, `inform`).
  Macros live in `MACRO_SPEC_REGISTRY` and are resolved in a
  separate beat-2 dispatch step. Keeping these registries
  distinct means the primary coinflip pool can never accidentally
  pick a macro, and macro resolution can never accidentally
  short-circuit a primary handler.
- **Macros are not optional once the voter declared one.** If
  the voter's action_packet contains a `>>` tail, every macro in
  that tail fires when the voter wins the cycle. The user's
  intent ("this voter being relevant means these substrings need
  surfacing") gets honoured every cycle; speech-act phrasing
  variation lives in the primary coinflip, not in whether the
  macro fires.
- **Macros do not compute.** They extract substrings or look up
  memories, then substitute into a template. If the caller wants
  computation over the extracted substring (e.g. actually
  evaluating `2+2 = 4`), that happens downstream — in an AIML
  rule, in a primary handler, or in a follow-up cycle. The macro
  surface stays mechanical.
- **AIML is not modified except for one keyword arg.**
  `generate_aiml_payload` gains `extra_support_pieces`. No new
  envelope, no skeleton dispatcher, no two-channel render. The
  resolved macro string is a `String` in `support_pieces`,
  identical in shape to a triple-derived string.
...
2. **Empty-resolution semantic.** When a macro's regex matches
   ...
   `"<no match for calculate>"` so AIML can voice-over its absence?
...
4. **Built-in seed set.** Three macros seed at `__init__()`:
   `calculate`, `timeOf`, `reflect`. Add `greet`, `recallTopic`,
   `dateOf`? Or keep the seed set minimal and let users register
   their own?

5. **Inhibition strings on macro invocations.** Earlier revisions
   allowed inhibitions (`[don't show work]`) on macro entries.
   The new model has no concept of "weight" on a macro (they all
   fire unconditionally). Inhibitions could still be passed
   through to AIML's existing inhibition router. Phase 1: drop
   inhibitions on macro tails entirely, force voters to put them
   on the primary half. Confirm.

6. **`MACRO_SPEC_REGISTRY` scope.** Specimen-local (per-specimen
   macros), or global (cross-specimen)? Phase 1: specimen-local,
   following the same convention as nodes.

7. **`:relational_triples` scope (deferred to phase 2).** A macro
...
except for one keyword arg. The whole macro runtime is one small
module, ~150 lines including JSON parsing, scope dispatch,
template substitution, and slash command surface.*
