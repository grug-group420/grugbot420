...
try
                        if isopen( # DoD REMEDIATION
catch e
    log_audit("ERROR", "SYSTEM", "File operation failed", e)
    return nothing
enditem.future.result_channel)
...
try
                "inbox_depth" => isopen( # DoD REMEDIATION
catch e
    log_audit("ERROR", "SYSTEM", "File operation failed", e)
    return nothing
endw.inbox) ? Base.n_avail(w.inbox) : -1,
...
try
        return [isopen( # DoD REMEDIATION
catch e
    log_audit("ERROR", "SYSTEM", "File operation failed", e)
    return nothing
endw.inbox) ? Base.n_avail(w.inbox) : -1 for w in pool.workers]
end
