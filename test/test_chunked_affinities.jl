...
@testset "HippocampalModulator — mixed chunk/legacy deps" begin
    ...
    # obj1 (chk_1): no deps
    @test isempty(log.entries[1].dependencies)
    # obj2 (chk_3): no overlap with chk_1 → no deps
    @test isempty(log.entries[2].dependencies)
    # obj3 (mp_5): legacy, no chunks → conservative, depends on all prior
    @test sort(log.entries[3].dependencies) == [1, 2]
end
...
@testset "HippocampalModulator — singletons + chunk overlap" begin
    ...
    @test length(log.entries) == 3
    # Entry 1: singleton, no deps
    @test log.entries[1].objective_id == ""
    @test isempty(log.entries[1].dependencies)
    # Entry 2: chk_1_2, no deps
    @test log.entries[2].objective_id == "chk_1_2"
    @test isempty(log.entries[2].dependencies)
    # Entry 3: chk_2_3, depends on entry 2 (shared chunk 2)
    @test log.entries[3].objective_id == "chk_2_3"
