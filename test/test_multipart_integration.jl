...
@testset "Multipart Integration — singleton input → one objective" begin
    ...
    v = TestVote("node_rock", "reason_rock", 0.7, "", :singleton)
...
@testset "Multipart Integration — mixed singleton + multipart votes" begin
    ...
    v_mp1 = TestVote("node_fire", "reason_fire", 0.85, "mp_1", :primary)
    v_mp2 = TestVote("node_ice",  "reason_ice",  0.78, "mp_2", :primary)

    # Singleton vote from an image specimen (or a node that didn't decompose).
    v_singleton = TestVote("node_img", "describe_image", 0.60, "", :singleton)
...
@testset "Multipart Integration — group with primary and supports" begin
    ...
    primary  = TestVote("node_a", "explain_physics", 0.85, "mp_1", :primary)
    s_lock   = TestVote("node_b", "note_gravity",    0.83, "mp_1", :support)
    s_unsure = TestVote("node_c", "note_momentum",   0.55, "mp_1", :support)
    # Below threshold — should be dropped entirely.
    s_drop   = TestVote("node_d", "note_irrelevant", 0.05, "mp_1", :support)
...
@testset "Multipart Integration — and-no-split stays singleton" begin
    ...
    v = TestVote("node_food", "describe_food", 0.6, "", :singleton)
    objs = build_objectives([v])
...
@testset "Multipart Integration — two groups plus singleton" begin
    ...
    v1 = TestVote("node_x",   "explain_x",    0.80, "mp_1", :primary)
    v2 = TestVote("node_y",   "explain_y",    0.75, "mp_2", :primary)
    v3 = TestVote("node_img", "describe_img", 0.60, "",     :singleton)
